...
public class SubmitThreadPool implements BeanFactoryAware {
    ...
    final RejectedExecutionHandler handler = new RejectedExecutionHandler() {
        ...
        public void rejectedExecution(Runnable r, ThreadPoolExecutor executor) {
            //加入到缓冲队列
            AnswerSheetDTO answerSheetDTO = ((BusinessThread) r).getAnswerSheetDTO();
            msgQueue.offer(answerSheetDTO);
            System.out.println("系统繁忙, 考试放入调度线程池，答卷ID:" + answerSheetDTO.getAnswerSheetId());
        }
    };
     ...
     * 将 提交答卷 任务加入线程池
     */
    public void addOrders(AnswerSheetDTO answerSheetDTO) {
        // 验证当前进入的考生是否已经存在提交记录
        ...
        if (!redisUtil.exists(id)) {
            redisUtil.set(id, "1");
            BusinessThread businessThread = new BusinessThread(answerSheetDTO);
            threadPool.execute(businessThread);
        } else {
            throw new BusinessException(500, "已经提交过了");
        }
    }
        ...
        public void run() {
            ...
            if (!msgQueue.isEmpty()) {
                ...
                if (threadPool.getQueue().size() < WORK_QUEUE_SIZE) {
                    ...
                    BusinessThread businessThread = new BusinessThread(answerSheetDTO);
                    threadPool.execute(businessThread);
                    logger.warn("(调度线程池)缓冲队列出现考试业务，重新添加到线程池，答卷ID: {}", answerSheetDTO.getAnswerSheetId());
                }
            }
        }
